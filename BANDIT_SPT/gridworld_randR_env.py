...
class Gridworld_RandReward_4x4_OneHot_Env(gym.Env):

    def __init__(self):
        super(Gridworld_RandReward_4x4_OneHot_Env, self).__init__()
        self.distance = 0
        self.reward_range = (-1, 0)
        self.action_space = spaces.Discrete(4)
        # although there are 2 terminal squares in the grid
    ...
    def step(self, action):
        ...
        self.obs = next_obs
        info = {}
        next_obs_onehot = np.zeros(self.observation_space.n)
        next_obs_onehot[next_obs] = 1
        return next_obs_onehot, reward, done, info
        # return next_obs, reward, done, info
...
class Gridworld_RandReward_8x8_OneHot_Env(gym.Env):

    def __init__(self):
        super(Gridworld_RandReward_8x8_OneHot_Env, self).__init__()
        self.distance = 0
        self.reward_range = (-1, 0)
        self.action_space = spaces.Discrete(4)
        # although there are 2 terminal squares in the grid
...
class Gridworld_RandReward_16x16_OneHot_Env(gym.Env):

    def __init__(self):
        super(Gridworld_RandReward_16x16_OneHot_Env, self).__init__()
        self.distance = 0
        self.reward_range = (-1, 0)
        self.action_space = spaces.Discrete(4)
        # although there are 2 terminal squares in the grid
        ...
        for s in self.gridworld.flat[1:self.observation_space.n]:
            ...
            for a, d in zip(
                ...
                next_row = max(0, min(row + d[0], 15))
                next_col = max(0, min(col + d[1], 15))
                s_prime = self.gridworld[next_row, next_col]
                self.P[a, s, s_prime] = 1
    ...
    def reset(self):
        ...
        #self.obs = 6
        self.obs = np.random.randint(1, self.observation_space.n, size=1)
        self.distance = ( int(self.obs/16) + self.obs%16 )
        # self.obs
        next_obs_onehot = np.zeros(self.observation_space.n)
        next_obs_onehot[self.obs] = 1
        return next_obs_onehot
...
class Gridworld_RandReward_32x32_OneHot_Env(gym.Env):

    def __init__(self):
        super(Gridworld_RandReward_32x32_OneHot_Env, self).__init__()
        self.distance = 0
        self.reward_range = (-1, 0)
        self.action_space = spaces.Discrete(4)
        self.gridwidth = 32
        ...
        # they are considered as 1 state
        # therefore observation is between 0 and 14
        self.observation_space = spaces.Discrete(self.gridwidth*self.gridwidth)
        
        self.gridworld = np.arange(
                self.observation_space.n
                ).reshape((self.gridwidth, self.gridwidth))
        #self.gridworld[-1, -1] = 0
        ...
        for s in self.gridworld.flat[1:self.observation_space.n]:
            ...
            for a, d in zip(
                ...
                next_row = max(0, min(row + d[0], self.gridwidth-1 ))
                next_col = max(0, min(col + d[1], self.gridwidth-1 ))
                s_prime = self.gridworld[next_row, next_col]
                self.P[a, s, s_prime] = 1
    ...
    def reset(self):
        ...
        #self.obs = 6
        self.obs = np.random.randint(1, self.observation_space.n, size=1)
        self.distance = ( int(self.obs/self.gridwidth) + self.obs%self.gridwidth )
        # self.obs
        next_obs_onehot = np.zeros(self.observation_space.n)
        next_obs_onehot[self.obs] = 1
        return next_obs_onehot
...
class Gridworld_RandReward_1x1_OneHot_Env(gym.Env):
...
class Gridworld_banditcase_RandReward_Env(gym.Env):

    def __init__(self):
        super(Gridworld_banditcase_RandReward_Env, self).__init__()
        self.distance = 0
        ...
        # they are considered as 1 state
        # therefore observation is between 0 and 14
        self.observation_space = spaces.Discrete(1) #0 start 0 end
