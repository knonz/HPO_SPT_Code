...
def finish_episode(episode,past_prob_table):
    ...
    gd_counter = 0
    flip_num = int (args.flipped * env.observation_space.n*ACTION_DIM)
    print("flip_num",flip_num)
    deltaYs = []
    seen = set()
    for itemp in range(flip_num+1):
        # dy_temp = [0]* env.observation_space.n*ACTION_DIM
        dy_temp = np.full(env.observation_space.n*ACTION_DIM,0)
        dy_temp[0:0+itemp] = 1
        temp3 = list(permutations(dy_temp))
        
        for itemp2 in range(len(temp3)):
            
            temp4 = list(temp3[itemp2])
            
            if temp3[itemp2] not in seen:
                print(temp4)
                seen.add(temp3[itemp2])
                deltaYs.append(  temp4 )
        # deltaYs.append(list(permutations(dy_temp)))
        # deltaYs = permutations([1, 2, 3])
    print("deltaYs",deltaYs)
    while True:
        
        # rc_losses = torch.tensor()
        rc_losses =torch.ones([len(deltaYs)], dtype=torch.float64 )

        # for idx in range(len(returns)):
        # for deltaY in range(len(deltaYs)):
        rcCounter = 0
        for deltaY in deltaYs :
            ...
            for si in range(  env.observation_space.n):
                ...
                for aj in range( ACTION_DIM ):
                    ...
                    # pdb.set_trace()
                    # print("(1-2*deltaY[idx])",(1-2*deltaY[idx]))
                    policy_loss = yweight[idx] * ploss( x1 , x2 , ys[idx]*(1-2*deltaY[idx]) )
                    ...
                    # writer.add_scalar("Reward/a{}_probabilityInwhileLoop".format(aj), probs[aj].item() , gd_counter)
                        # writer.add_scalar("Reward/a{}_actionheadInwhileLoop".format(aj), model_ahead[aj]  , gd_counter)
            # rcloss = torch.stack(policy_losses).sum()
            rc_losses[rcCounter] = torch.stack(policy_losses).sum()
            rcCounter+=1
            # rc_losses.append(rcloss)
            
        # reset gradients
        # hard_update(model, old_model)
        # rc_losses = torch.tensor(rc_losses)
        loss = torch.max(rc_losses)
